    ...
    <div className="p-6 space-y-6 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
      ...
      <div className="relative w-full max-w-md">
        ...
        >
          ...
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-4.3-4.3"></path>
        </svg>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg shadow-lg bg-white dark:bg-gray-800 transition-colors duration-300">
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-100 dark:bg-gray-700">
            <tr>
              <th className="px-4 py-3 border-b border-gray-300 dark:border-gray-600">#</th>
              <th className="px-4 py-3 border-b border-gray-300 dark:border-gray-600">Name</th>
              <th className="px-4 py-3 border-b border-gray-300 dark:border-gray-600">Email</th>
              <th className="px-4 py-3 border-b border-gray-300 dark:border-gray-600">Role</th>
              <th className="px-4 py-3 border-b border-gray-300 dark:border-gray-600">Admin Action</th>
              <th className="px-4 py-3 border-b border-gray-300 dark:border-gray-600">Delete</th>
            </tr>
          </thead>
              ...
              <tr key={user._id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                ...
                <td className="px-4 py-3 flex items-center gap-3">
                  <div className="w-12 h-12 rounded-lg overflow-hidden">
                    <img src={user.photoURL} alt="avatar" className="object-cover w-full h-full" />
                  </div>
                ...
                <td className="px-4 py-3">
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${user.role === 'admin' ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-200' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200'}`}>
                    {user.role || 'user'}
                  </span>
                </td>
                <td className="px-4 py-3 space-x-2">
                  {user.role === 'admin' ? (
                    <button onClick={() => handleRemoveAdmin(user)} className="px-3 py-1 bg-yellow-500 hover:bg-yellow-400 text-white rounded-lg transition">
                      <FiShieldOff />
                    </button>
                  ) : (
                    <button onClick={() => handleMakeAdmin(user)} className="px-3 py-1 bg-green-500 hover:bg-green-400 text-white rounded-lg transition">
                      <FaUserShield />
                    </button>
                  )}
                </td>
                <td className="px-4 py-3">
                  <button onClick={() => handleDeleteUser(user._id)} className="px-3 py-1 bg-red-500 hover:bg-red-400 text-white rounded-lg transition">
                    <FaTrash />
                  </button>
                </td>
              </tr>
